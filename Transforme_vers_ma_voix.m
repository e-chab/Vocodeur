function y = Transforme_vers_ma_voix(x, Fs, cible_file)
% Transforme une voix source pour se rapprocher d'une voix cible.
% cible_file est optionnel : un fichier par defaut est choisi si disponible.
...
if nargin < 3 || isempty(cible_file)
    ...
    if isempty(cible_file)
        error('Aucun fichier cible trouve pour l''effet Transforme_vers_ma_voix.');
    end
end
