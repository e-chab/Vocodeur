...
function plot_superimposed(x, y, Fs, effectName, plotParams)
    % Affiche les signaux avant et après superposés et exporte la figure
    global figDir
    fig = figure('Name', ['Effet: ' effectName ' - Signal'], 'Visible', 'off');
    t = (0:length(x)-1) / Fs;
    ...
    outPath = fullfile(figDir, ['superimposed_' effectName '.png']);
    exportgraphics(fig, outPath, 'Resolution', 300);
    close(fig);
end

function plot_spectrum(x, y, Fs, effectName)
    % Affiche les spectres de puissance avant et après et exporte la figure
    global figDir
    fig = figure('Name', ['Effet: ' effectName ' - Spectre'], 'Visible', 'off');
    subplot(2, 1, 1);
    [pxx, f] = pwelch(x, [], [], [], Fs);
    plot(f, 10*log10(pxx));
    grid on;
    ...
    xlabel('Fréquence (Hz)');
    ylabel('Puissance (dB/Hz)');
    outPath = fullfile(figDir, ['spectrum_' effectName '.png']);
    exportgraphics(fig, outPath, 'Resolution', 300);
    close(fig);
end

function plot_spectrogram(x, y, Fs, effectName)
    % Affiche les spectrogrammes avant et après et exporte la figure
    global figDir
    fig = figure('Name', ['Effet: ' effectName ' - Spectrogramme'], 'Visible', 'off');
    win = hamming(512);
    noverlap = 256;
    nfft = 1024;
    subplot(2, 1, 1);
    ...
    outPath = fullfile(figDir, ['spectrogram_' effectName '.png']);
    exportgraphics(fig, outPath, 'Resolution', 300);
    close(fig);
end
